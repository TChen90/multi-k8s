...
kubectl apply -f k8s
kubectl set image deployments/server-deployment server=tchen90/multi-server:$SHA
kubectl set image deployments/client-deployment server=tchen90/multi-client:$SHA
kubectl set image deployments/worker-deployment server=tchen90/multi-worker:$SHA